...
def encode_segmentation_rgb(use_mask_face_part_id_list, segmentation, no_neck=True):
    assert type(use_mask_face_part_id_list) == list

    print (use_mask_face_part_id_list)
    parse = segmentation
    # if use_mask_face_part_id_list == 'None' :
    #     # face_part_ids = [1, 2, 3, 4, 5, 6, 10, 12, 13] if no_neck else [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 13, 14]
    #     print ('DEFAULT')
    # else: 
    face_part_ids = use_mask_face_part_id_list if no_neck else [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 13, 14]


        #problem là ['1,2,3,4,5'] , cái cần là [1,2,3,4,5]
    #  print ('NON DEFAULT')
...
def reverse2wholeimage(b_align_crop_tenor_list,swaped_imgs, mats, crop_size, oriimg, logoclass, save_path = '', \
                    no_simswaplogo = False,pasring_model =None,norm = None, use_mask = False, use_mask_face_part_id_list = None):
    ...
    for swaped_img, mat ,source_img in zip(swaped_imgs, mats,b_align_crop_tenor_list):
        ...
        if use_mask:
            ...
            tgt_mask = encode_segmentation_rgb(use_mask_face_part_id_list, vis_parsing_anno)
...
####resize like normal 224 image in simswap 
                print (swaped_img.shape)
                swaped_img = cv2.resize(swaped_img, (224, 224))
                print (swaped_img.shape)

                
                target_image_parsing = postprocess(swaped_img, source_img[0].cpu().detach().numpy().transpose((1, 2, 0)), target_mask,smooth_mask)
                #target_image_parsing == the mask of use mask 
        ...
        if use_mask:
            target_image = np.array(target_image, dtype=np.cfloat) * 255
        else:
            target_image = np.array(target_image, dtype=np.cfloat)[..., ::-1] * 255
    ...
    # target_image /= 255
    # target_image = 0
    img = np.array(oriimg, dtype=np.cfloat)
